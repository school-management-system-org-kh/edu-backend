...
const SubjectList = ({ loading, data, tableParams, setTableParams, getListSubject, setId, setLoading, setkeyword }) => {
    const messageModalRef = useRef('');
    const { t } = useTranslation();
    const today = dayjs().format("MM-DD-YYYY");

    const exportToExcel = (data, fileName = "subject_lists.xlsx") => {
    ...
    const exportToPDF = (columns, data, fileName = "subject_lists.pdf") => {
        const doc = new jsPDF();
  ...
  `;
    ...
    const tableSubjectMangement = {
        ...
        columns: [
        {
            title: <span style={{fontWeight:600}}>Subject Name</span>,
            dataIndex: "name",
            render: (text) => (text && <span style={{ textTransform: "capitalize" }}>{text}</span>),
            key: "name",
        ...
        {
            title: <span style={{fontWeight:600}}>Subject Code</span>,
            dataIndex: "code",
            key: "code",
            sorter: (a, b) => a.code.localeCompare(b.code), // Proper string sorting
            sortDirections: ["ascend", "descend"], // show sort icons
            showSorterTooltip: false, // removes tooltip, keeps icon
        },
        {
            title: <span style={{fontWeight:600}}>Subject Type</span>,
            dataIndex: "type",
            key: "type",
            render: (text) => (text && <span style={{ textTransform: "capitalize" }}>{text}</span>),
            sorter: (a, b) => a.code.localeCompare(b.type), // Proper string sorting
            sortDirections: ["ascend", "descend"], // show sort icons
            showSorterTooltip: false, // removes tooltip, keeps icon
        },
        {
            ...
            >
                ...
                >
                    <div style={{ display: "flex", alignItems: "center" }}>
                        ...
                        <div
                            style={{ marginLeft: "0.5rem", cursor: "pointer" }}
                            onClick={() => exportToPDF(tableSubjectMangement.columns, data)}
                        >
                            <AiOutlineFilePdf size={18} />
                        </div>
                        <div
                            style={{ marginLeft: "0.5rem", cursor: "pointer" }}
                            onClick={() => printTable(tableSubjectMangement.columns, data)}
                        >
                ...
                <Spin spinning={loading}>
                    <div className="w-100">
                        <TableFetchData
                            ...
                            data={data}
                            columns={tableSubjectMangement.columns}
                            x={500}
                        />
                    </div>
                </Spin>
