...
const SectionList = ({ loading, data, tableParams, setTableParams, getListSection, setId, setLoading, setkeyword }) => {
    const messageModalRef = useRef('');
    const { t } = useTranslation();
    const today = dayjs().format("MM-DD-YYYY");
    const exportToExcel = (data, fileName = "section_lists.xlsx") => {
    ...
    const exportToPDF = (columns, data, fileName = "section_lists.pdf") => {
        const doc = new jsPDF();
            ...
            >
                ...
                >
                    <div style={{ display: "flex", alignItems: "center" }}>
                        ...
                        <div
                            style={{ marginLeft: "0.5rem", cursor: "pointer" }}
                            onClick={() => exportToPDF(tableSectionMangement.columns, data)}
                        >
                            <AiOutlineFilePdf size={18} />
                        </div>
                        <div
                            style={{ marginLeft: "0.5rem", cursor: "pointer" }}
                            onClick={() => printTable(tableSectionMangement.columns, data)}
                        >
                ...
                <Spin spinning={loading}>
                    <div className="w-100">
                        <TableFetchData
                            ...
                            data={data}
                            columns={tableSectionMangement.columns}
                            x={500}
                        />
                    </div>
                </Spin>
